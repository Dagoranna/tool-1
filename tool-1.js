if (document.readyState === "loading") {
  document.addEventListener("DOMContentLoaded", initExtention);
} else {
  initExtention();
}

const extensionDivID = "toolplace-1";
const activeDecoderObj = {
  id: "decoder_5",
  name: "HTML Escape",
...
//includeFileCSS("https://startling-lollipop-b709a4.netlify.app/tool-css.css");
includeFileCSS("tool-css.css");

const EXTENSION = {
  activeDecoder: "decoder_5",
};
...
EXTENSION.utils = {
  ...
  readContinuationByte: function () {
    if (this.byteIndex >= this.byteCount) {
      throw Error("Invalid byte index");
    }

    let continuationByte = this.byteArray[this.byteIndex] & 0xff;
    this.byteIndex++;
  ...
  utf8decode: function (byteString) {
    this.byteArray = EXTENSION.utils.ucs2decode(byteString);
  ...
  /*GraphemeSplitter - библиотека, не знаю как без нее обойтись

    //URL Encode
    //1 option: params.encodeNonSpec = "Encode Non-special Characters"
    urlEncodeText: function (params) {
...
*/
  ...
  textToHtmlEntities: function (params) {
    const entities = [];
    const text = params.text;
    ...
    for (let i = 0; i < codes.length; i++) {
      ...
      if (allowNamed && EXTENSION.CONST.NAMED_ENTITIES_BY_NUMBERS[code]) {
        entities.push(EXTENSION.CONST.NAMED_ENTITIES_BY_NUMBERS[code]);
      } else if (hex) {
  ...
  //HTML Unescape
  //no options
  htmlEntitiesToText: function (params) {
    const alphaIndex = {};
    const charIndex = {};
    const str = params.text;

    var i = EXTENSION.CONST.NAMED_ENTITIES_BY_NAMES.length;
    var _results = [];

    while (i--) {
      var e = EXTENSION.CONST.NAMED_ENTITIES_BY_NAMES[i];
      var alpha = e[0];
      var chars = e[1];
      var chr = chars[0];
      var addChar =
  ...
  textToHexadecimal: function (params) {
    const text = params.text;
  ...
  //HEX Decode
  //no options
  hexadecimalToText: function (params) {
  ...
  // params.customBase14,
  unicodeToCodePoints: function (params) {
  ...
  // params.customBase15,
  codePointsToUnicode: function (params) {
    const text = params.text;
...
EXTENSION.UI = {
  ...
  init: function () {
    this.panel = HtmlElement.getById(extensionDivID);

    EXTENSION.UI.inputsWrapper = HtmlElement.create({
      type: "div",
    ...
    }).appendTo(EXTENSION.UI.inputsWrapper);
    ...
    EXTENSION.UI.inputField = HtmlElement.create({
      type: "textarea",
      id: "ext-0x03-input-textarea",
      classes: "brwl-0x03-input ext-0x03-textarea-field",
      events: {
        blur: (e) => {
          EXTENSION.UI.outputField.setValue(
            EXTENSION.UI.decoderProcessing(
              EXTENSION.activeDecoder,
              e.target.value,
            ),
          );
        },
    ...
    EXTENSION.UI.outputField = HtmlElement.create({
      type: "textarea",
      id: "ext-0x03-output-textarea",
      classes: "brwl-0x03-input ext-0x03-textarea-field",
    }).appendTo(EXTENSION.UI.inputsWrapper);

    EXTENSION.UI.optionsWrapper = HtmlElement.create({
      type: "div",
      id: "ext-0x03-options-wrapper",
      classes: "ext-0x03-options-wrapper",
    }).appendTo(EXTENSION.UI.inputsWrapper);

    //EXTENSION.toolsJSON.forEach((item) => EXTENSION.UI.drawOptions(item));
    EXTENSION.UI.drawOptions(activeDecoderObj);
  },

  drawOptions: function (item) {
    ...
    const decoderElem = HtmlElement.create({
      ...
      styles: {
        display: EXTENSION.activeDecoder === item.id ? "flex" : "none",
      },
      classes: "ext-0x03-options-separate",
    }).appendTo(EXTENSION.UI.optionsWrapper);
            ...
            case "checkbox":
              currentElem = HtmlElement.create({
                ...
                .addChildWithLabel({
                  ...
                  events: {
                    change: () => EXTENSION.UI.decodeInputToOutput(),
                  },
  ...
  decoderProcessing: function (decoderId, text) {
    let output = "";
    switch (decoderId) {
      case "decoder_1":
        ...
        output = EXTENSION.utils.urlEncodeText({
          encodeNonSpec,
          text,
        });
        break;
      case "decoder_2":
        output = EXTENSION.utils.urlDecodeText({ text });
        break;
      case "decoder_3":
        ...
        output = EXTENSION.utils.textToBase64({ text, split, splitSize });
        break;
      case "decoder_4":
        output = EXTENSION.utils.base64ToText({ text });
        break;
      case "decoder_5":
        ...
        output = EXTENSION.utils.textToHtmlEntities({
          text,
          escapeSpecialOnly,
          useDecimalBases,
          printNameReferences,
        ...
        break;
      case "decoder_6":
        output = EXTENSION.utils.htmlEntitiesToText({ text });
        break;
      case "decoder_7":
        ...
        output = EXTENSION.utils.textToHexadecimal({ text, spaces, prefix });
        break;
      case "decoder_8":
        output = EXTENSION.utils.hexadecimalToText({ text });
        break;
      case "decoder_9":
        output = EXTENSION.utils.ROT13({ text });
        break;
      case "decoder_10":
        const squareBrackets = document.getElementById(
          "ext-0x03-decoder_10-square-brackets",
        ).checked;
        output = EXTENSION.utils.DefangURL({ text, squareBrackets });
        break;
      case "decoder_11":
        output = EXTENSION.utils.RefangURL({ text });
        break;
      case "decoder_12":
      ...
      case "decoder_14":
        ...
        output = EXTENSION.utils.unicodeToCodePoints({
          text,
          separator14,
          codePointBase14,
          customBase14,
      ...
      case "decoder_15":
        ...
        output = EXTENSION.utils.codePointsToUnicode({
          text,
          separator15,
          codePointBase15,
          customBase15,
      ...
      case "decoder_16":
        ...
        output = EXTENSION.utils.convertToSlug({
          text,
          multiline16,
          lowercase16,
        });
